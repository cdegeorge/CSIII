// Modified code by Mikhail Nesteranko to improve hashmap
...
public:
	...
	void setNumBuckets(size_t numBuckets) { numBuckets_ = numBuckets; }
...
template <typename Key, typename Value,
	...
	public:
		...
		void rehash(size_t n);									// resized the hashmap to the given size
		size_t size() const { return size_; }					// returns the size of the hashmap
	private:
...
hashmap<Key, Value, Compare, Hash>::hashmap(
	...
	elems_ = vector<list<Element>>(hash_.getNumBuckets());
}
...
pair<Value*, bool> hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
	...
	if (it == elems_[bucket].end()) {
		...
		returnedPair = make_pair(&(elems_[bucket].back().second), true);
		return returnedPair;
	}
	else {
		returnedPair = make_pair(&it->second, false);
		return returnedPair;
	}
}
...
pair<pair<const Key, Value>*, bool> hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
	pair<const Key, Value> nextElement;
	size_t bucket = hash_.hash(x);
	...
	if (it != elems_[bucket].end()) {    // the element exists, erase it
		...
		// Get element after one that is deleted
		auto nextIt = find_if(elems_.begin() + bucket, elems_.end(),
			[](list<pair<const Key, Value>> b) -> bool {
			return b.size() > 0;
		});

		// If element was last in container
		if (nextIt == elems_.end())
			return make_pair(nullptr, true);

		pair<const Key, Value> nextElement = *((*nextIt).begin());
		return make_pair(&nextElement, true);
	}
	return make_pair(&nextElement, false);
}

// Resized the haspmap to the given size
template <typename Key, typename Value, typename Compare, typename Hash>
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
	if (hash_.getNumBuckets() < n) { // Do nothing if n is smaller than size
		hash_.setNumBuckets(n);
		vector<list<Element>> tmp = move(elems_);

		elems_.clear();
		elems_.resize(n);
		size_ = 0;

		for (list<Element> b : tmp)
			for (Element e : b)
				insert(e);
	}
}
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
	...
	if (found == nullptr) { // if key not found, create new element with empty value
		pair<Value*, bool> check = insert(make_pair(x, Value())); // calling default constructor on Value
